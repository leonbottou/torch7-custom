...
static void nxn_(SpatialMaxPooling_updateOutput_frame)(real *input_p, real *output_p,
...
static int nxn_(SpatialMaxPooling_updateOutput)(lua_State *L)
{
  ...
  if (input->nDimension == 3)
  {
    ...
    nxn_(SpatialMaxPooling_updateOutput_frame)(input_data, output_data,
...
#pragma omp parallel for private(p)
    for (p = 0; p < nbatch; p++)
    {
      nxn_(SpatialMaxPooling_updateOutput_frame)(input_data+p*nslices*iwidth*iheight, output_data+p*nslices*owidth*oheight,
...
static void nxn_(SpatialMaxPooling_updateGradInput_frame)(real *gradInput_p, real *gradOutput_p,
                                                         real *indx_p, real *indy_p,
                                                         long nslices,
                                                         long iwidth, long iheight,
                                                         long owidth, long oheight,
...
static int nxn_(SpatialMaxPooling_updateGradInput)(lua_State *L)
{
  ...
  if (input->nDimension == 3)
  {
    nxn_(SpatialMaxPooling_updateGradInput_frame)(gradInput_data, gradOutput_data,
                                                 indices_data+nslices*owidth*oheight, indices_data,
                                                 nslices,
                                                 iwidth, iheight,
                                                 owidth, oheight,
...
#pragma omp parallel for private(p)
    for (p = 0; p < nbatch; p++)
    {
      nxn_(SpatialMaxPooling_updateGradInput_frame)(gradInput_data+p*nslices*iwidth*iheight, gradOutput_data+p*nslices*owidth*oheight,
                                                   indices_data+(p+nbatch)*nslices*owidth*oheight, indices_data+p*nslices*owidth*oheight,
                                                   nslices,
                                                   iwidth, iheight,
                                                   owidth, oheight,
...
static const struct luaL_Reg nxn_(SpatialMaxPooling__) [] = {
  {"SpatialMaxPooling_updateOutput", nxn_(SpatialMaxPooling_updateOutput)},
  {"SpatialMaxPooling_updateGradInput", nxn_(SpatialMaxPooling_updateGradInput)},
  {NULL, NULL}
};

static void nxn_(SpatialMaxPooling_init)(lua_State *L)
{
  luaT_pushmetatable(L, torch_Tensor);
  luaT_registeratname(L, nxn_(SpatialMaxPooling__), "nxn");
  lua_pop(L,1);
}
