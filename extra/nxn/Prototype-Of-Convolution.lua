...
function ComputeGradWeights(gradOutput, input, kernel, parms)
   ...
   -- create the gradWeights matrix
   --local kcopy = copySpatialConvolutionKernel(kernel,reverse):zero()
   local kcopy = newSameTensor(kernel, (#kernel)[1], (#kernel)[2], (#kernel)[3], (#kernel)[4]):zero()
   ...
   -- copy gradOutput into ocopy
   local ocopy = newSameTensor(gradOutput, bs, toh, tow, op)
   ocopy:fill(0)
   
   local tocopy = ocopy:narrow(2,1,oh):narrow(3,1,ow)
   tocopy:copy(gradOutput)
